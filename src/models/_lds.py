...
Steps:
  1. Bin training labels into fixed-width buckets (physical MW grid).
...
def compute_lds_weights(
    ...
    Args:
        labels:         1-D array of target values (MW).
        bin_width:      Physical width of each bin in the same units as labels (MW).
                        Bins are anchored to multiples of bin_width, so the grid is
                        consistent across different training subsets.
        ks:             Gaussian kernel size (must be odd).
    ...
    lo = np.floor(labels.min() / bin_width) * bin_width
    hi = np.ceil(labels.max()  / bin_width) * bin_width
